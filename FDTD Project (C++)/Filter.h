...
class Filter {
  ...
  public:
    // Constructor
    Filter(DM da_nodes, Vec x, PetscInt filterT, PetscScalar radiusVec[6], PetscScalar weightVec[6]);
  ...
  private:
    ...
    PetscInt    filterType; // Filter type: 0=sens, 1=dens, val == no filtering
    PetscScalar R_M[3][3];  // Radius filter matrix
    PetscScalar W_M[3][3];  // Weight matrix (w_xx, w_xy, w_xz, ...)

    // Setup datastructures for the filter
    PetscErrorCode SetUp(DM da_yee_lattice, Vec x);
